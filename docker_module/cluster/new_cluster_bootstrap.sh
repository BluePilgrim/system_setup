...
docker run -d -p 2181:2181 -p 2888:2888 -p 3888:3888 --name=zookeeper -e SERVER_LIST=${PRIMARY_SERVER} -e ZK_ID=1 cluster/hadoop /usr/local/bin/zookeeper_bootstrap.sh
...
docker run -d -p 5051:5051 --name=mesos_slave1 --net=host --privileged -v /var/run/docker.sock:/var/run/docker.sock -v /sys:/sys -e PUBLIC_IP=${PRIMARY_SERVER} -e ZOOKEEPER=${ZOOKEEPER} -e MESOS_SLAVE_OPTS="${MESOS_SLAVE_OPTS}" cluster/hadoop /usr/local/bin/mesos-slave_bootstrap.sh
echo "Created mesos slave..."

# create three mesos masters
#QUORUM_VAL=2
#QUORUM_VAL=1
docker run -d -p 5050:5050 --name=mesos_master1 --net=host -e PUBLIC_IP=${PRIMARY_SERVER} -e ZOOKEEPER=${ZOOKEEPER} -e QUORUM_VAL=${QUORUM_VAL} -e NAME="Mesos Master 1" -e PORT=5050 -e MESOS_MASTER_OPTS="${MESOS_MASTER_OPTS}" cluster/hadoop /usr/local/bin/mesos-master_bootstrap.sh
echo "Created mesos master 1..."

docker run -d -p 15050:5050 --name=mesos_master2 --net=host -e PUBLIC_IP=${PRIMARY_SERVER} -e ZOOKEEPER=${ZOOKEEPER} -e QUORUM_VAL=${QUORUM_VAL} -e NAME="Mesos Master 2" -e PORT=15050 -e MESOS_MASTER_OPTS="${MESOS_MASTER_OPTS}" cluster/hadoop /usr/local/bin/mesos-master_bootstrap.sh
echo "Created mesos master 2..."

docker run -d -p 25050:5050 --name=mesos_master3 --net=host -e PUBLIC_IP=${PRIMARY_SERVER} -e ZOOKEEPER=${ZOOKEEPER} -e QUORUM_VAL=${QUORUM_VAL} -e NAME="Mesos Master 3" -e PORT=25050 -e MESOS_MASTER_OPTS="${MESOS_MASTER_OPTS}" cluster/hadoop /usr/local/bin/mesos-master_bootstrap.sh
echo "Created mesos master 3..."

# create one marathon
docker run -d -p 9090:9090 --name=marathon -e PUBLIC_IP=${PRIMARY_SERVER} -e ZOOKEEPER=${ZOOKEEPER} -e MARATHON_OPTS="${MARATHON_OPTS}" cluster/hadoop /usr/local/bin/marathon_bootstrap.sh
